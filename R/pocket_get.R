...
#' parse_item_
#' @description parse item returned as a list into a mini tibble.
#' @param item Pocket item as returned by the retrieve endpoint
#' @value tibble.
#' @keywords internal
...
pocket_get <- function(consumer_key,
  ...
  if (res$status_code == 400) {
    print("Invalid request syntax, please contact the package maintainers.")
  }
  ...
  if (res$status_code == 403) {
    print(
      "Authentication successful, but access denied due to lack of permission or rate limiting."
    )
  }
  if (res$status_code == 503) {
    print("Pocket's sync server is down for scheduled maintenance. Try again later.")
  }
  ...
  if (all_items == FALSE) {
    to.be.retrieved <-
      c(c("item_id", "given_url", "given_title"), add_item)
  } else{
